...
- `lib/services/firebase_auth_service.dart`: Firebase Auth email/password + Google sign-in, email verification send, password reset email, profile editing (name, email via verification flow), and ID token retrieval
...
- `.github/workflows/android-aab-prerelease-manual.yml`: manual (`workflow_dispatch`) signed Android AAB build + GitHub prerelease creation, attaches `app-release.aab` for Play Store uploads
...
- `flutter analyze` warning cleanup: removed an unused optional `padding` parameter from private `_DetailsSectionCard` in `lib/pages/book_details_page.dart`.
- Book details page fixed a header overlap regression by enabling top safe-area padding for the scroll content when using the fixed `CupertinoNavigationBar` (`SliverSafeArea(top: true)`).
...
- OpenLibrary direct flow now sends an explicit `User-Agent` header and uses a 2-step request pattern: one `/search.json` request during search, then one `/works/{id}.json` details request only when the user chooses to add an OpenLibrary result (no re-search on add).
